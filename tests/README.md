...
Core tests take longer than any other Electroneum tests, due to the high amount of computational work involved in validating core components.
...
To run only Electroneum's core tests (after building):
...
To run only Electroneum's crypto tests (after building):
...
```bash
bydotcoind --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `bydotcoin-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Electroneum's hash tests (after building):
...
To run only Electroneum's performance tests (after building):
...
To run only Electroneum's unit tests (after building):
